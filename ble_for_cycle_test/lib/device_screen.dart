...
import 'package:ble_for_cycle_test/controller/esp/esp_controller.dart';
import 'package:ble_for_cycle_test/controller/setting/setting_controller.dart';
import '../model/models/cycle_model.dart';

/// ESP <-> App Connection 3단계
class ConnectToDevice {
  /// connection이 일어남. 단, 기기와 mapping까지는 되지 않고, device name으로만 connect함.
  Future<void> connectToDevice (EspController espController) async {
    print('*******************.....CONNECTING.....*********************');
    if (!espController.isConnected) {
...
/// ESP <-> App Connection 4단계
/// UUID와 Rx, Tx를 가지고 최종 mapping을 하고,
/// 컨트롤러에 값을 초기화해주는 부분
class DiscoverServices {
  SettingController settingController = Get.find<SettingController>();
  //ModelController modelController = Get.find<ModelController>();
  DateTime dateTimeMemoization = DateTime.now(); // for safety
  var resultofcycle;
  Future<void> discoverServices(EspController espController) async {
    int count=0;
    print("******************DISCOVER SERVICES**************************");

    // 서비스 리스트 중 내 기기에 맞는걸? 찾아야함.
    List<BluetoothService> services = await espController.targetDevice.discoverServices();
    services.forEach((service) {
      if (service.uuid.toString() == settingController.selectedUuidValue) {
        count++;
        service.characteristics.forEach((characteristic) {
          // print(characteristic.uuid);
          if (characteristic.uuid.toString() == settingController.selectedCharRxValue) {
            print("RX 감지덕지");
            count++;
            espController.targetCharacteristicRx = characteristic;
            espController.espInit();
          }
          else if (characteristic.uuid.toString() == settingController.selectedCharTxValue) {
            print("TX 감지");
            count++;
            espController.targetCharacteristicTx = characteristic;
            if (!espController.isListener) {
              espController.isListener=true;
              /// ESP에서 들어오는 값에 대한 반응을 해주는 부분
              characteristic.setNotifyValue(true);
              int countCheck=0;

              espController.notifyStream = characteristic.value.listen((value) async {
                /// 값을 받은 경우
                if (value.length >0) {

                  // Cycle
                  if (settingController.selectedType==4) {
                    espController.cycleModel.value = CycleModel.fromString(String.fromCharCodes(value));
                    resultofcycle = espController.cycleModel.value;
                  }
                }
              });
            }
...
class _DeviceScreenState extends State<DeviceScreen> {
  ...
  DiscoverServices discoverServices = Get.find<DiscoverServices>();
  ...
  Widget characteristicInfo(BluetoothService r) {
    ...
    //discoverServices.resultofcycle.toString()
    return Text(name + discoverServices.resultofcycle.toString());
  }
